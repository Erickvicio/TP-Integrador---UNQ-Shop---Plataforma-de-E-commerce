...
class TransferenciaBancariaTest {
    ...
    void testValidarCvu_CuandoElCvuEsCero_LanzaExcepcion() {
        ...
        // 2. Act & Assert: Cambiado a referencia de método (sin lambdas ni llaves)
    ...
    @Test
    void testValidarDatos_CuandoLaApiRechazaLaTransferencia_LanzaExcepcion() {
        // 1. Arrange: Datos válidos para pasar los métodos privados de arriba
        transferenciaBancaria.setcvu(123456);
        transferenciaBancaria.setAlias("alias.valido");

        // Forzamos a que la API devuelva false
        when(apiTransferenciaMock.validarDatos(123456, "alias.valido")).thenReturn(false);

        // 2. Act & Assert: Usamos referencia de método para evitar bloques lambda huérfanos
        Exception excepcion = assertThrows(RuntimeException.class, transferenciaBancaria::validarDatos);

        // 3. Verificaciones extras de la cátedra
        assertEquals("Error: La API externa rechazó la transfencia Bancaria.", excepcion.getMessage());
        verify(apiTransferenciaMock).validarDatos(123456, "alias.valido");
    }
}