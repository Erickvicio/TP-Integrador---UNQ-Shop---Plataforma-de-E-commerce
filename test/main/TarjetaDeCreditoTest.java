...
class TarjetaDeCreditoTest {
    ...
    void setUp() {
        ...
        // Tenés que pasarle el mock por constructor para que no sea null adentro
    ...
    void testValidarDatos_CuandoElVencimientoEsNull_LanzaExcepcionYNoLlamaALaApi() {
        // 1. Arrange: Provocamos el error pasando null
        tarjetaDeCredito.setNumero(123456);
        tarjetaDeCredito.setCvv(123);
        tarjetaDeCredito.setVencimiento(null); 

        // 2. Act & Assert: Atrapamos la excepción usando referencia de método (sin lambdas)
        Exception excepcion = assertThrows(RuntimeException.class, tarjetaDeCredito::validarDatos);

        // 3. Verificamos el mensaje que quería el profesor (sin meterlo adentro del assertThrows)
        assertEquals("Error: La fecha de vencimiento es nula.", excepcion.getMessage());
        
        // 4. Clave: Nos aseguramos de que no gaste recursos llamando a la API
